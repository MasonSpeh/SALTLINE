...
## Realistic model-based crab: a photoreal deep-sea crab mesh generated by Meshy AI.
## Static mesh for now (rig pending); behavior and AI unchanged. Lamp still lives as a
## separate light that pulses with hunt state.
...
func _build_body() -> void:
	if MODEL == null:
		# Fallback: if GLB didn't import, use primitives (shouldn't happen).
		...
		_build_legs()
		return
	# Load the photoreal mesh.
	_model = MODEL.instantiate()
	# Normalise scale: the GLB is ~0.9m across at its longest axis.
	var bounds: AABB = _model.get_child(0).get_aabb() if _model.get_child_count() > 0 else AABB()
	var longest: float = maxf(bounds.size.x, maxf(bounds.size.y, bounds.size.z))
	var scale: float = 0.9 / longest if longest > 0.01 else 1.0
	_model.scale = Vector3.ONE * scale
	add_child(_model)
	# Carapace rim glow spots — dim until the lamp hunts (separate, still procedural).
	for i in range(6):
...
func _animate(delta: float) -> void:
	...
	if not _model:
		...
		# Realistic model: simple bob + sway while moving.
		if speed > 0.1:
			var bob: float = sin(_bob_t * 4.0) * 0.02
			var sway: float = sin(_bob_t * 1.8) * 0.05
			_model.position.y = bob
			_model.rotation.z = sway * 0.1
		else:
