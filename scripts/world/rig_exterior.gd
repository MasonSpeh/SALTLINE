...
## horizontal axis. A rail run ends in a square cap, and where two catwalk runs meet at a
## right angle a capsule cutting the corner diagonally hits both caps in the same frame; the
## two slide directions cancel and the player stops dead a hand's width from the rail.
## Pulling the invisible barrier back from the visual steel rounds that corner off, and it
## cannot open a hole — a player capsule needs 0.74 m of clear width to pass anything.
