...
## way. So this rays every candidate azimuth back to the subject and takes the first one with
## a clear line, reporting which — and reporting that ALL of them were blocked if they were,
## instead of silently saving a picture of a wall.
func _look_at_clear(centre: Vector3, back: float, tag: String) -> void:
	...
	var tried: int = 0
	for lift in [0.34, 0.62, 0.12]:
		for i in range(8):
			var a: float = i * TAU / 8.0
			var dir: Vector3 = Vector3(cos(a), lift, sin(a)).normalized()
			...
			if not hit.is_empty() and (hit["position"] as Vector3).distance_to(centre) > 0.3:
				continue
			await _look_from(centre, back, dir, tag)
			print("[deeprig]   (clear vantage found on try %d, azimuth %.0f°, lift %.2f)"
				% [tried, rad_to_deg(a), lift])
			return
	print("[deeprig] %s: NO CLEAR VANTAGE in %d tries — shooting the default anyway" % [tag, tried])
...
func _pole_shots() -> void:
	...
	if found != null:
		...
		await _look_at_clear(mid, 2.30, "pole_world")
		await _look_at_clear(mid + Vector3(0, -0.10, 0), 1.15, "pole_world_close")
	else:
	...
	await get_tree().process_frame
	for spot in [[Vector3(20.0, 2.4, -25.6), 190.0, -6.0], [Vector3(27.0, 2.3, -20.0), 0.0, -8.0]]:
